...
| ------------------------ | ---------------------------------------------- | ------------------------------------------ |
| `host-types`             | TypeScript declarations only — `AtriumRegistry`, `UserContext`, `NavItem` / `AdminTab` / `ProfileItem`, `AtriumNotification`, `AtriumEvent`, `AtriumLocation`. | Type-checking host bundles against the registry contract. No runtime code. |
| `host-bundle-utils`      | Vite preset (`./vite`), React hooks (`./react` — `useMe` / `usePerm` / `useRole` / `useAtriumLocation` / `useAtriumNavigate`), `__atrium_t__` i18n helper, the dual-tree mount pattern. | Building a host bundle: `main.tsx` ends up ~10 lines of `register*` calls. |
